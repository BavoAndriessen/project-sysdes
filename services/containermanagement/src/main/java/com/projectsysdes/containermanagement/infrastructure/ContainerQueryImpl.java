...
public class ContainerQueryImpl implements ContainerQuery {
    ...
    @Override
    public ContainerLocationViewModel getContainerLocation(Integer containerId) throws ContainerNotFoundException {
        ContainerDataModel cdm = repo.findById(containerId).orElseThrow(ContainerNotFoundException::new);
        return new ContainerLocationViewModel(cdm.getCurrentLocationType(), cdm.getCurrentLocationIdentifier());
    }
    ...
    public List<ContainerViewModel> searchContainersForContents(String contents) {
        return repo.findByContentsLike("%"+contents+"%").stream()
                .map(c -> {
                    return new ContainerViewModel(c.getContainerId(), c.getContents(), c.getState(), new ContainerLocationViewModel(c.getCurrentLocationType(), c.getCurrentLocationIdentifier()), new ContainerLocationViewModel(c.getDestinationLocationType(), c.getDestinationLocationIdentifier()));
                })
